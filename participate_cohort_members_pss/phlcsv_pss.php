...
class phl_csv{
    ...
    public function phl_load_csv_content($content, $encoding, $delimiter_name, $column_validation=null, $enclosure='"') {
        ...
        if (!isset($columns[0])) {
            ...
            // UPDATED 06/03/2023
            if(empty($columns[1][2])||!$columns[1][2]){
                redirect($CFG->wwwroot.'/phlcohort/training_schedule/upload_diemdanh_pss.php?empty_cell=1');
            }   else {
                $cohort = $DB=>get_record_sql(
                    'SELECT id 
                    FROM {cohort} 
                WHERE idnumber = ?',array('idnumber'=>$columns[1][2]));
                if(empty($cohort->id)||!$cohort->id){
                    redirect($CFG->wwwroot.'/phlcohort/training_schedule/upload_diemdanh_pss.php?incorrect_cohort=1');
                }   else    {
                    ...
                    WHERE c.idnumber = ?',array('idnumber'=>$columns[1][2]));
                    if(empty($cohort_members->cmid)||!$cohort_members->cmid){
                        redirect($CFG->wwwroot.'/phlcohort/training_schedule/upload_diemdanh_pss.php?empty_cohort=1');
                    }
                    /* 1. CHECK IF USERNAME WITHIN FILE IS NOT NULL */
                    $starting_cell = 4; $addedUser = array(); $cohortMemberId = array();
                    for($i=4;$i<(count($cohort_members)+3);$i++){
                        $members = $DB->get_record_sql(
                            'SELECT cm.id cmid
                            FROM {cohort_members} cm
                            INNER JOIN {cohort} c on c.id = cm.cohortid 
                            INNER JOIN {user} u on u.id = cm.userid
                        WHERE c.idnumber = ? and u.username = ?',array('idnumber'=>$columns[1][2],'username'=>$columns[$starting_cell][5]));
                        if(empty($members->cmid)||!$members->cmid){
                            redirect($CFG->wwwroot.'/phlcohort/training_schedule/upload_diemdanh_pss.php?null_user='.$columns[$starting_cell][5]);
                        }   else    {
                            $addedUser[$starting_cell] = $columns[$starting_cell][5];
                            $cohortMemberId[$starting_cell] = $members->cmid;
                            $starting_cell ++;
                        }
                    }
                    for($i=4;$i<(count($cohort_members)+3);$i++){
                        if(empty($columns[$i][9])||!$columns[$i][9]){
                            $columns[$i][9] = '';
                        }
                        if(empty($columns[$i][10])||!$columns[$i][10]){
                            $columns[$i][10] = '';
                        }
                        if(empty($columns[$i][11])||!$columns[$i][11]){
                            $columns[$i][11] = '';
                        }
                        if(empty($columns[$i][12])||!$columns[$i][12]){
                            $columns[$i][12] = '';
                        }
                        if(empty($columns[$i][13])||!$columns[$i][13]){
                            $columns[$i][13] = '';
                        }
                        if(empty($columns[$i][14])||!$columns[$i][14]){
                            $columns[$i][14] = '';
                        }
                        if(empty($columns[$i][17])||!$columns[$i][17]){
                            $columns[$i][17] = '';
                        }
                    }

                    /*
                    $total_members = count($cohort_members);
                    for($i=4;$i<$total_members+3;$i++){
                        if(empty($columns[$i][9])||!$columns[$i][9]){
                            $columns[$i][9] = '';
                        }
                        if(empty($columns[$i][10])||!$columns[$i][10]){
                        ...
                        $username = $DB->get_record_sql('SELECT ')
                    }*/
                }
            }
            // END
