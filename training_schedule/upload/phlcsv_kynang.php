...
class phl_csv{
    ...
    public function phl_load_csv_content($content, $encoding, $delimiter_name, $column_validation=null, $enclosure='"') {
        ...
        if (!isset($columns[0])) {
            ...
            //
            for($i=6;$i<=$count_members+5;$i++){
                if(empty($columns[$i][7])){
                    $columns[$i][7] = '';
                }
                if(empty($columns[$i][8])){
                    $columns[$i][8] = '';
                }
...
";
                ...
                $a0 = render_user_check_mark($columns[$i][6]);
                $a1 = render_user_check_mark($columns[$i][7]);
                $a2 = render_user_check_mark($columns[$i][8]);
                $a3 = render_user_check_mark($columns[$i][9]);
                $a4 = render_user_check_mark($columns[$i][10]);
                $a5 = $columns[$i][11];
                $sql = "
            ...
            set ";
                ...
                {
                    $sql .= ", participate_condition = N'$a4'";
                }
                if($a5=='')
                {
                    $sql .= ", note = NULL";
...
<th style='color:white;'>Ghi chú</th>
  ...
  ";
            for($i=6;$i<=$count_members+5;$i++){
                ...
                echo "<td>".$columns[$i][2]."</td>";
                echo "<td>".$columns[$i][6]."</td>";
                echo "<td>".$columns[$i][7]."</td>";
                echo "<td>".$columns[$i][8]."</td>";
                echo "<td>".$columns[$i][9]."</td>";
                echo "<td>".$columns[$i][10]."</td>";
                echo "<td>".$columns[$i][11]."</td></tr>";
            }
